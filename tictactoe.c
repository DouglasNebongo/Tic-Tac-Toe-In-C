...
void display_grid(TBoard A) {
    ...
    for (int i = 0; i < ROWS; i++) {
        printf("%d |", i + 1); // Display row numbers and left border
        for (int j = 0; j < COLS; j++) {
    ...
    // Print bottom border of the grid
    printf("  +");
    for (int j = 0; j < COLS; j++) {
        printf("--");
    }
...
void play_game(TBoard A) {
    ...
    do {
        ...
        bool align = is_aligned(A); // Check if there is an alignment (win condition)
        if ((align == true) && (disc == 'X')) {
            printf("\nGame Over! Player 1 wins!\n");
            stop = 1; // End the game
        } else if ((align == true) && (disc == 'O')) {
            printf("\nGame Over! Player 2 wins!\n");
            stop = 1; // End the game
        } else if ((align == true) && !((disc == 'X') || (disc == 'O'))) {
